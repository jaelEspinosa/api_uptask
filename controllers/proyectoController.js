...
const editarProyecto = async (req, res) => {
  ...
  proyecto.cliente = req.body.cliente || proyecto.cliente
   try {
     const proyectoAlamacenado = await proyecto.save()
     res.json(proyectoAlamacenado)
   } catch (error) {
    console.log(error)
   } 
}
...
const agregarColaborador = async (req, res) => {
 ...
 // Comprobar que el colaborador no esté ya en el proyecto

 if(proyecto.colaboradores.includes(usuario._id)){
  const error = new Error ('Este colaborador ya está en el proyecto')
   return res.status(404).json({msg: error.message})
 }

 // esta todo ok agregamos el colaborador
...
const eliminarColaborador = async (req, res) => {
  ...
  // esta todo ok eliminamos el colaborador
